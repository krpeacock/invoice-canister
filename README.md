...
[View this sample's code on GitHub](https://github.com/krpeacock/invoice-canister)
...
The main project demonstrates the support of four different tokens, two of which use the types of the ICP standard and two of which use the types of the ICRC1 standard. A simpler version of the same interface integrating support for only two tokens, one ICP, and the other ICRC1, can be found in the [examples/motoko-seller-client] subdirectory. 
...
This is only a summary description of the general functionality of the Invoice Canister, review the [design doc](https://github.com/krpeacock/invoice-canister/docs/DesignDoc.md) for more details in particular being aware of the security concerns such as regarding an invoice's data privacy. Additionally, there is extensive commentary in this [invoice.did](https://github.com/krpeacock/invoice-canister/invoice.did), or the [Invoice.mo](https://github.com/krpeacock/invoice-canister/src/invoice/Invoice.mo), [Types.mo](https://github.com/krpeacock/invoice-canister/src/invoice/modules/Types.mo) and the associated modules of the [SupportedToken.mo](https://github.com/krpeacock/invoice-canister/src/invoice/modules/supported-token/SupportedToken.mo) files.
...
### Step 1: Navigate into the folder containing the project's files and install the required packages:
...
public type SupportedToken<T1, T2> = {
  ...
  // T1 is used for any tokens using the ICP specification, while T2 for any of the ICRC1 specifications.
...
Be sure to update each reference of the tag. For the `#ICRC1_ckTESTBTC` tag that was added above, updating the switch in `getEncodedInvoiceSubaccountAddress()` would look like this:
